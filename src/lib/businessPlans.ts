...
export const BUSINESS_PLANS: BusinessPlan[] = NEEDARO_PLANS.map((plan) => ({
    ...
    activeJobLimit: plan.id === "free" ? 1 : plan.id === "starter" ? 5 : 99,
    activeJobs: plan.id === "free" ? "1 active request" : plan.id === "starter" ? "5 active requests" : "Unlimited active requests",
    deliveryTargetHours: 24,
    deliveryTarget: "Reply fast to nearby requests",
